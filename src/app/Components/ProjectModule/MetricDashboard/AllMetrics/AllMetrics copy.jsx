...
const AllMetrics = (props) => {
  ...
  useEffect(() => {
    ...
    if(props.selectedOption?.name && props.selectedOption.name ==='year'){
      console.log('year')
      API.get(`/metric?pillar_id=${props.pillarId}&fiscal_year=${props.selectedOption?.value && props.selectedOption.value}`, {
        headers: {
      ...
      console.log('qtr')
      API.get(`/metric?pillar_id=${props.pillarId}&fiscal_qtr=${props.selectedOption?.value && props.selectedOption.value}`, {
        headers: {
          'Authorization': 'Bearer ' + token
        }
      }).then(response1=>{
